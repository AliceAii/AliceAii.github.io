...
This is the **identification problem** --- translating causal (counterfactual) quantities into expressions involving only the observational distribution $P(X, Z, W, Y)$. The key insight is that the structure of the Standard Fairness Model, combined with the **counterfactual calculus** (ctf-calculus) and the **Ancestral Multi-World Network** (AMWN) --- tools developed in [Correa & Bareinboim (2024)](https://proceedings.mlr.press/v267/correa25a.html) --- allows us to do exactly this through a principled, step-by-step derivation.
