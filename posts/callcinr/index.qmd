...
1. Estimate the relationship using Nadaraya-Watson kernel regression
2. Compute bootstrap confidence intervals for the regression curve
3. Visualize the results with uncertainty bands
...
- **Numerical stability**: Check for `denominator > 0` before division
- **NA handling**: Return `NA_REAL` for grid points with no nearby data

### Loading the Data

We'll use petroleum tax and consumption data to illustrate the method:

```{r load_petroleum_data}
...
cat("Tax range: [", min(X), ",", max(X), "]\n")
cat("Consumption range: [", min(Y), ",", max(Y), "]\n")
```

### Computing the Kernel Regression Estimate

```{r compute_kernel_regression}
n <- length(X)
...
1. **Resample**: Draw n pairs $(X_i, Y_i)$ with replacement from the original data
2. **Refit**: Compute kernel regression on the bootstrap sample
3. **Repeat**: Do this B = 200 times
4. **Quantiles**: At each grid point, compute the 2.5th and 97.5th percentiles
...
upper_ci <- apply(boot_mat, 2, quantile, probs = 0.975, na.rm = TRUE)
```

**Why bootstrap works here:**
